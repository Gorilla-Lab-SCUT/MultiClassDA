...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000):
        ...
        self.fc = nn.Linear(512 * block.expansion, num_classes)  ## for classification
        self.fc_aux1 = nn.Linear(512 * block.expansion, num_classes)  ## auxiliary classifier one
        self.fc_aux2 = nn.Linear(512 * block.expansion, num_classes)  ## auxiliary classifier two
        self.fcdc = nn.Linear(512 * block.expansion, 1)           ## domain classifier
    ...
    def forward(self, x, alpha):
        ...
        x = x.view(x.size(0), -1)
        out = self.fc(x)
        rev_x = ReverseLayerF.apply(x, alpha)  ## gradient reverse
        out1 = self.fc_aux1(rev_x)
        out2 = self.fc_aux2(rev_x)
        outdc = self.fcdc(rev_x)

        trunc_x = ZeroLayerF.apply(x, 0.0)  ## zero gradient
        out1_trunc = self.fc_aux1(trunc_x)
        out2_trunc = self.fc_aux2(trunc_x)

        return x, out, out1, out2, outdc, out1_trunc, out2_trunc
...
def resnet18():
    ...
    if cfg.MODEL.PRETRAINED:
        ...
        pretrained_dict_temp.pop('fc.weight')
        pretrained_dict_temp.pop('fc.bias')
